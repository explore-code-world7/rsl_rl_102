...
class Hand3_Actor(nn.Module):
    def __init__(self,  num_actor_obs,
        ...
        super(Hand3_Actor, self).__init__()
        self.num_actions = num_actions
        self.num_actor_obs = num_actor_obs
        mlp_input_dim_a = num_actor_obs

        global_layers = []
        global_layers.append(nn.Linear(mlp_input_dim_a, global_dims[0]))
        global_layers.append(activation)
        print(f"global_dims = {global_dims}")

        for l in range(len(global_dims)-1):
            global_layers.append(nn.Linear(global_dims[l], global_dims[l + 1]))
            global_layers.append(activation)
        self.global_extractor = nn.Sequential(*global_layers)
...
class AC_Hand(nn.Module):
    ...
    def __init__(self,  num_actor_obs,
        ...
        self.actor = Hand3_Actor(num_actor_obs, num_actions, activation, num_leg_actions, num_hand_actions, num_rest_actions,\
